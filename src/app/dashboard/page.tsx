...
function DashboardContent() {
  ...
  useEffect(() => {
    if (visible) { const timer = setTimeout(() => setVisible(false), 5000); return () => clearTimeout(timer); }
  }, [visible]);
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between text-left">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <button className="relative p-2 text-slate-500 hover:text-slate-900 transition cursor-pointer"><Bell size={20} /><span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border border-white"></span></button>
            <button onClick={goToStartProject} className="hidden md:flex items-center gap-2 bg-slate-900 hover:bg-black text-white px-4 py-2 md:px-5 md:py-2.5 rounded-xl text-sm font-bold shadow-md transform active:scale-95 transition cursor-pointer font-sans"><Plus size={16} /> Start Project</button>
...
>
  <User size={16} className="text-slate-400" /> View Profile
                            ...
                            <div className="border-t border-slate-50" />
                            <div className="py-1"><button className="w-full flex items-center gap-3 px-4 py-2 text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition cursor-pointer text-left font-sans font-medium"><Settings size={16} className="text-slate-400"/> Account Settings</button><button className="w-full flex items-center gap-3 px-4 py-2 text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition cursor-pointer text-left font-sans font-medium"><HelpCircle size={16} className="text-slate-400"/> Help & Feedback</button></div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-8 text-left">
        ...
        <main className="flex-1 text-left">
          {visible && (
            <div className="mb-6 bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500 shadow-sm">
              <div className="flex items-center gap-3"><div className="w-7 h-7 bg-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-xs">✓</div><p className="text-[13px] md:text-sm font-bold text-emerald-900 font-sans tracking-tight">Project published successfully!</p></div>
              <button onClick={() => setVisible(false)} className="text-emerald-400 hover:text-emerald-600 cursor-pointer p-1 transition"><X size={18}/></button>
            </div>
          )}
          <div className="flex items-center justify-between mb-8 text-left">
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 tracking-tight font-sans">{activeFilter}</h1>
            <div className="flex items-center gap-2"><span className="text-[10px] font-bold text-slate-400 hidden md:block uppercase tracking-widest font-sans">Sort by:</span><select className="bg-white border border-slate-200 text-xs md:text-sm rounded-lg px-3 py-1.5 font-bold text-slate-700 outline-none cursor-pointer focus:ring-2 focus:ring-slate-100 transition shadow-sm font-sans"><option>Newest First</option></select></div>
          </div>
...
// 4. THE WRAPPER EXPORT
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 flex items-center justify-center font-bold text-slate-400 animate-pulse tracking-widest italic text-xs uppercase font-sans">Connecting to Hub...</div>}>
      <DashboardContent />
    </Suspense>
  );
}