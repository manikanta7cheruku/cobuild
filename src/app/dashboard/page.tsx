  ...
  useEffect(() => {
    async function getData() {
      ...
      if (authUser) {
        ...
        // Fetch Projects + Owner Profile Data for Unified Search
    ...
    <div className="bg-slate-50 text-slate-900 min-h-screen pb-24 md:pb-0 font-['Plus_Jakarta_Sans',_sans-serif]">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between text-left">
          ...
          {/* SEARCH BAR (UNIFIED) */}
          <div className="hidden md:block flex-1 max-w-md mx-8 text-left">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            {/* USER MENU DROPDOWN (FIXED SCROLLING & SIZE) */}
                    ...
                    <div className="absolute right-0 top-11 md:top-12 w-64 md:w-80 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden transform origin-top-right transition-all z-50">
                        ...
                        <div className="max-h-[calc(100vh-6rem)] overflow-y-auto no-scrollbar">
                                    ...
                                    <div className="bg-[#F8FAFC] pb-3 px-5 space-y-4 animate-in fade-in duration-200">
                                        ...
                                        <div className="pt-2 text-left">
                                            ...
                                            {projects.filter(p => p.owner_id === user?.id).length > 0 ? (
                                                projects.filter(p => p.owner_id === user?.id).map(p => (
                                                    <div key={p.id} className="bg-white border border-slate-200 rounded-lg p-2.5 shadow-sm mb-2 group">
                                                        <div className="flex justify-between items-center mb-1">
                                                            ...
                                                            <span className="text-[9px] bg-red-50 text-red-500 px-1.5 rounded-full font-bold">3 Apps</span>
                                                        </div>
                                                ...
                                                <div className="py-1">
                                                    <p className="text-[11px] text-slate-400 leading-relaxed italic">Ready to take the lead? Share your idea.</p>
                                                    <button onClick={goToStartProject} className="text-[11px] text-blue-600 font-bold mt-1 hover:underline cursor-pointer">+ Start a Project</button>
                                                </div>
                                            )}
                                        </div>
                            ...
                            <div className="py-1">
                                ...
                                <button className="w-full flex items-center gap-3 px-5 py-2.5 text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition cursor-pointer text-left"><Bell size={18} className="text-slate-400"/> Notifications</button>
        ...
        <div className={`md:hidden bg-slate-50 border-b border-slate-200 overflow-y-auto transition-all duration-300 ${mobileFiltersOpen ? 'max-h-[60vh] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="p-4 space-y-6 text-left">
                ...
                <button onClick={toggleMobileFilters} className="w-full py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm cursor-pointer">Apply Filters</button>
            </div>
        </div>
      </nav>
        ...
        <main className="flex-1 text-left">
          ...
          {visible && (
            <div className="mb-6 bg-emerald-50 border border-emerald-100 p-3 md:p-4 rounded-2xl flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 md:w-8 md:h-8 bg-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-xs">✓</div>
                <p className="text-[13px] md:text-sm font-bold text-emerald-900 font-sans">Project published successfully!</p>
              </div>
          ...
          <div className="flex items-center justify-between mb-8 text-left">
            ...
            <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-slate-400 hidden md:block uppercase tracking-widest font-sans">Sort by:</span>
                <select className="bg-white border border-slate-200 text-xs md:text-sm rounded-lg px-3 py-1.5 font-bold text-slate-700 outline-none cursor-pointer focus:ring-2 focus:ring-slate-100 transition shadow-sm font-sans">
          ...
          {loading ? (
            <div className="py-20 text-center font-bold text-slate-400 animate-pulse text-xs tracking-widest uppercase italic font-sans">Gathering projects...</div>
                ...
                <div key={project.id} className="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-lg transition duration-300 cursor-pointer group flex flex-col h-full shadow-sm">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        {/* RELATIVE TIME POSITIONED BESIDE BADGE */}
                  ...
                  <div className="mb-auto text-left">
                    <h2 className="text-[17px] md:text-[18px] font-bold text-slate-900 mb-2 leading-tight group-hover:text-blue-600 transition tracking-tight font-sans">{project.name}</h2>
                    <p className="text-[13px] md:text-[14px] text-slate-500 mb-5 line-clamp-2 leading-relaxed font-medium font-sans">{project.pitch}</p>
                    
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {project.tech_stack?.map((tech: string) => (
                        <span key={tech} className="px-2 py-0.5 bg-slate-100 text-slate-500 border border-slate-200 rounded text-[9px] font-bold uppercase tracking-tight group-hover:bg-blue-50 transition-colors font-sans">#{tech}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-4 mt-2 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-slate-600 font-bold uppercase tracking-tight font-sans">Project Lead</span>
                    </div>
                    
                    {/* SPOTS OPEN WITH GREEN INDICATOR */}
            ...
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-12 md:p-24 flex flex-col items-center justify-center text-center shadow-sm text-left">
                ...
                <h2 className="text-xl md:text-2xl font-bold text-slate-900 mb-3 tracking-tight font-sans">No projects yet</h2>
                <p className="text-slate-500 max-w-sm mb-10 text-[14px] md:text-[16px] font-medium leading-relaxed font-sans text-center">When you or your squad create projects, they’ll appear here. Use this space to showcase what you’re building and find the right teammates.</p>
                <button onClick={goToStartProject} className="inline-flex items-center gap-2 px-8 py-3 bg-slate-900 hover:bg-black text-white text-sm font-bold rounded-xl shadow-lg transition active:scale-95 cursor-pointer font-sans tracking-tight">Start your first project</button>
            </div>
          )}
        </main>
      </div>
      ...
      <div className="md:hidden fixed bottom-6 right-6 z-50">
        <button onClick={goToStartProject} className="bg-slate-900 text-white w-12 h-12 rounded-full shadow-xl flex items-center justify-center active:scale-105 transition shadow-blue-900/30 cursor-pointer">
          <Plus size={24} />
        </button>
      </div>
    </div>
...
// FINAL EXPORT WITH SUSPENSE FOR VERCEL
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 flex items-center justify-center font-bold text-slate-400 animate-pulse tracking-widest italic text-xs uppercase font-sans">Connecting to Cobuild...</div>}>
      <DashboardContent />
    </Suspense>
  );
}